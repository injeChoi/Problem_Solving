...
/*
 ...
 Swift에서는 ceil(), 타입 캐스팅 등 다른 요소도 고려할 점이 있기 때문에
 시간복잡도에 좀 더 유의해야 한다
...
func solution(_ n: Int, _ stations: [Int], _ w: Int) -> Int {
    var answer = 0, start = 1, end = 0
    let dist = Double(2 * w + 1)

    for station in stations {
        end = station - w - 1
        let gap = end - start + 1
        answer += Int(ceil(Double(gap) / dist))
        start = station + w + 1
    }

    answer += Int(ceil(Double(n - start + 1) / dist))
    return answer
}
